...
class AppConfig:
    ...
    APP_VERSION = "1.0.2"  # FINAL - Self-contained deployment
    APP_DESCRIPTION = "State-of-the-art ML pipeline for house price prediction with 90.4% accuracy"
    BUILD_ID = "2024-09-08-FINAL"  # Force complete cache refresh
    ...
    # Self-contained deployment paths (all files within repository)
    PATHS = {
        'champion_model': Path('models/best_model.pkl'),
        'train_data': Path('data/final_train_prepared.csv'),
        'test_data': Path('data/final_test_prepared.csv'),
        'phase7_results': Path('data/metrics/optimization_results.json'),
        'final_evaluation': Path('data/metrics/final_evaluation.json'),
        'model_metadata': Path('data/model_metadata.json'),
        'feature_importance': Path('data/interpretability/global_feature_importance.json'),
        'partial_dependence': Path('data/interpretability/partial_dependence_analysis.json'),
        'business_insights': Path('data/insights/business_insights_analysis.json'),
        'phase8_visualizations': Path('assets/visualizations/')
    }
    ...
    def get_absolute_path(cls, path_key: str) -> Path:
        """Get absolute path for self-contained deployment."""
