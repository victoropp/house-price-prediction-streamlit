...
def show_model_analytics(data_loader, visualizer):
    ...
    with col1:
        # Get actual accuracy from model performance metrics
        accuracy = model_metrics.get('accuracy', 0.0)
...
def show_model_interpretation(data_loader, visualizer):
    ...
    st.info("📊 Understanding which features drive house prices across all predictions")
...
def show_quick_prediction_interface(data_loader, visualizer, model, train_data, feature_columns):
    ...
    with col1:
        ...
        if 'GrLivArea_transformed' in train_data.columns:
            ...
            gr_liv_area_log = st.number_input(
                ...
                step=100,
                help="Living area above ground level"
            )
        ...
        if 'TotalBsmtSF_transformed' in train_data.columns:
            ...
            total_bsmt_sf = st.number_input(
                "Total Basement Area (normalized)",
        ...
        garage_area = st.number_input(
            ...
            step=0.01,
            help="Normalized garage area (0-1 scale, ~0.34 = typical 2-car garage)"
        )
    ...
    with col2:
        ...
        overall_qual = st.selectbox(
            ...
            options=sorted(train_data['OverallQual'].unique()),
            index=len(sorted(train_data['OverallQual'].unique()))//2
        )
        
        # Check if OverallCond exists, otherwise use a reasonable default
        if 'OverallCond' in train_data.columns:
            overall_cond = st.selectbox(
                "Overall Condition (1-10)", 
                options=sorted(train_data['OverallCond'].unique()),
                index=len(sorted(train_data['OverallCond'].unique()))//2
            )
        else:
            overall_cond = st.selectbox(
                "Overall Condition (1-10)", 
        ...
        # Use encoded kitchen quality if available
        if 'KitchenQual_encoded' in train_data.columns:
            kitchen_qual_options = sorted(train_data['KitchenQual_encoded'].unique())
            kitchen_qual = st.selectbox(
                "Kitchen Quality (Encoded)",
                options=kitchen_qual_options,
                index=len(kitchen_qual_options)//2
            )
        else:
            kitchen_qual = st.selectbox(
                "Kitchen Quality",
    ...
    with col3:
        ...
        # Use encoded neighborhood if available
        if 'Neighborhood_encoded' in train_data.columns:
            neighborhood_options = sorted(train_data['Neighborhood_encoded'].unique())
    ...
    if st.button("🎯 **Get Price Prediction**", type="primary"):
        ...
        try:
            ...
            if prediction < 50000:  # Log-transformed, need to exp
            ...
            st.markdown(f"""
                <div class='prediction-result'>
                    ...
                    accuracy = model_metrics.get('accuracy', 0.0)
                    accuracy_pct = f"{accuracy*100:.1f}%" if accuracy > 0 else "N/A"
                    st.write(f"                    <p style='color: {config.COLORS['text']};'>🎯 Confidence Level: {accuracy_pct} (Cross-Validated)</p>", unsafe_allow_html=True)
                </div>
            """, unsafe_allow_html=True)
            
            # Get top feature importance for quick explanation
            feature_importance = data_loader.load_feature_importance()
            if feature_importance and 'shap_importance' in feature_importance:
                st.markdown("#### 🔍 Key Prediction Drivers")
                
                # Show top 5 features that are relevant to current prediction
                top_features = data_loader.get_top_features(n=5, method='shap_importance')
                
                for i, (feature, importance) in enumerate(top_features):
                    if feature in input_data:
                        feature_value = input_data[feature]
                        st.markdown(f"**{i+1}. {feature}**: {feature_value} (Impact: {importance:.3f})")
...
def show_advanced_prediction_interface(data_loader, visualizer, model, train_data, feature_columns):
    ...
    if st.button("🎯 **Generate Advanced Prediction**", type="primary"):
        try:
            ...
            # Transform prediction back if needed
            if prediction < 50000:
                prediction = np.exp(prediction)
